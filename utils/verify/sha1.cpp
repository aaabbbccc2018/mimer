...
void sha1::_sha1_update(sha1_t *ctx, const void *data, size_t size)
{
    size_t  used, free;
    ...
    if (size >= 64) {
        data = sha1_body(ctx, (const u_char *)data, size & ~(size_t) 0x3f);
        size &= 0x3f;
    }
...
void sha1::_sha1_final(u_char result[20], sha1_t *ctx)
{
    size_t  used, free;
...
const u_char* sha1::sha1_body(sha1_t *ctx, const u_char *data, size_t size)
{
